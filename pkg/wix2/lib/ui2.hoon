    ...
    e.forEach((el) => \{
        ...
        if (domEl) \{
            ...
            listeners.forEach((listener) => \{
                ...
                domEl.addEventListener(listener, function e(event) \{eventPoke(listener, event)})
            })
        }
        else \{
...
++  agent
  ...
  +*  this  .
      ...
      ag-state    on-save:ag
      pg    (~(page page-cor [components-state ag-state bowl]) ~(manx (~(got by components-state) "root") [ag-state ~ [bowl "root" ~]]))
      io    ~(. agentio bowl)
      morph-fact  (fact:io [%tape !>((en-xml:html in-body:pg))] ~[/morph])
  ::
  ++  on-poke
    |=  [=mark =vase]
    :: ::::~&  ag-state
    ...
    ?.  ?=(?(%ui %handle-http-request) mark)  ::TODO  i might be able to remove this check
        ...
        =^  c-cards  components-state  (image-to-instance:hc ~(manx (~(got by components-state) "root") [ag-state ~ [bowl "root" ~]]) |)
        ::~&  "c-cards"
        ::~&  c-cards
    ...
    ?-  -.poke
      %domevent
        ?-  +<.poke
          ...
          :: ::::~&  morph-fact
            [cards-out this(components-state state-update)]
  ...
  ++  on-init
    ...
    [(weld cards -.init-state) this(current-manx manx, components-state +.init-state, dom-event-listeners event-listeners, nid-map (~(put by nid-map) 'i1' [~.random-number "seed"]))]   ::TODO process incoming components to extract ids for nid-map ::TODO refactor sail inp
...
++  give-ids  ::TODO rename
  ...
  =/  new-c=sail-component
    ...
    ++  manx
      =/  man  ~(manx c vase children bowl)
        =^  with-ids  tic  (apply-elem man generate-id tic)
...
:: props are in sail props attribute as jammed noun encoded as tape
  ...
  ^-  (quip card:agent:gall component-instances)
    ::~&  "image-to-instance"
    =/  tic  0
    ...
    =/  c-i
      ...
      |_  [=vase children=marl bowl=sail-bowl]
        ...
        ++  manx  ;;(^^manx ~(manx c-w-ids [vase children bowl]))
      --
    ...
    =/  updated-c
      |_  [=vase children=marl bowl=sail-bowl]
        ...
        ++  on-poke  ~(on-poke c [vase children bowl])
        ++  manx
          ;;(^^manx (merge-manx ~(manx c (need prev-props-vase) ^children [^bowl (need cid) ref]) ~(manx c vase ^children [^bowl (need cid) ref])))
        --
    =.  components-state
      ?.  (~(has by components-state) (need cid))
    ...
    :: $(flattened t.flattened, components-state (~(uni by components-state) inner-components-state))
      ...
      :: $(flattened t.flattened)
    =/  inner-components-state=(quip card:agent:gall component-instances)
    ...
    $(flattened t.flattened, components-state (~(uni by components-state) +.inner-components-state), cards :(weld cards c-cards -.inner-components-state))
...
::
  ...
  ++  merge-attr
    ...
    --
  ++  apply-elem
    |=  [a=manx b=$-([marx @] [marx @]) tic=@]
    ^-  [manx @]
    :: ::~&  "apply-elem called"
    :: ::~&  "marl is:"
    :: ::~&  marl
    :: ::~&  "apply-elem called, manx is:"
    :: ::~&  a
    :: =/  rng  ~(. og (mix tic eny.bowl))
    :: =|  id-s=(set @)
    :_  tic
    |^  ^-  manx
    :: ::~&  "marl at recurse"
    :: ::~&  marl
    :: =/  no-of-mane
    ::   (no-of-mane marl n.g.a)
    :: |-
    :: =^  rn  rng  (rads:rng eny.bowl)
    :: ::~&  "no-of-mane"
    :: ::~&  no-of-mane
    ...
    ++  cloop
      ...
      :: ::~&  "new-marl"
      :: ::~&  new-marl
      [^$(a i.c, tic +(tic)) (cloop(tic +(tic)) t.c)]
    ::
    :: ++  no-of-mane
    ::   |=  [=marl =mane]
    ::   :: ::~&  "no-of-mane called"
...
++  generate-id
  |=  [=marx tic=@]  ^-  [^marx @]
  ?:  ?=(%$ n.marx)  [marx tic]
  ?:  ?=(html-tag:html-tag n.marx)  [marx tic]
  :: ::~&  "marx"
  :: ::~&  marx
  ?:  (~(has by (malt a.marx)) %sail-id)
  :: ::~&  "has sail-id, skipping"
  [marx tic]
  :: =/  id  (add (mug n.marx) tic)
  =/  id  (add (mug n.marx) tic)
  =.  a.marx
